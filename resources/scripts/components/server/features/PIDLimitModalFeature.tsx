        ...
        <Modal visible={visible} onDismissed={() => setVisible(false)} closeOnBackground={false} showSpinnerOverlay={loading}>
            ...
            {isAdmin ?
                <>
                    ...
                    <p css={tw`mt-4`}>在wings配置中增加<code css={tw`font-mono bg-neutral-900`}>container_pid_limit</code>, <code css={tw`font-mono bg-neutral-900`}>config.yml</code>，可能有助于解决这个问题。</p>
                    <p css={tw`mt-4`}><b>注意：必须重新启动 Wings 才能使配置文件更改生效</b></p>
                ...
                <>
                    ...
                    <p css={tw`mt-4`}><code css={tw`font-mono bg-neutral-900`}>pthread_create 失败，可能内存不足或已达到进程/资源限制</code></p>
                    <div css={tw`mt-8 sm:flex items-center justify-end`}>
                        <Button onClick={() => setVisible(false)} css={tw`w-full sm:w-auto border-transparent`}>
                            关闭
                        </Button>
