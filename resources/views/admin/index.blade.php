...
<div class="row">
    <div class="col-xs-12">
        ...
        ">
            ...
            <div class="box-body">
                @if ($version->isLatestPanel())
                    您正运行 Pterodactyl-CHINA 最新翼龙面板版本 <code>{{ config('app.version') }}</code>. 此版本为开发版。
                @else
                    您目前使用的面板 <strong>并非最新版!</strong> 目前最新版本为 <a href="https://github.com/Pterodactyl/Panel/releases/v{{ $version->getPanel() }}" target="_blank"><code>{{ $version->getPanel() }}</code></a> 您正运行的版本为 <code>{{ config('app.version') }}</code>. 此版本为开发版。
                @endif
            </div>
        </div>
    </div>
